...
最开始的问题确实是为什么这个so文件的汇编指令会被执行，但是在证明的过程中，又引出了其他的问题。为什么我更改dynamic段的文件偏移p_offset的值为0x58后再次载入该so文件会出错，为什么dynamic段的d_tag=0x5项的d_ptr更改为其他的值就会出错，这个项指向的地址根本是无意义的啊。同理d_tag=0x6的d_ptr指向的地址为0x0，同样更改后载入该so文件也会出错。为什么就不能替换成dynamic段其他必要的类型的项呢？

这些问题同样需要解决。稍后一段时间我会再次去调试，尝试解决。噢还得验证是否可以作为模板使用，因为汇编指令只从0x3A到0x4F，如果更长呢？使用jmp跳到最后面继续添加指令，这个文件是否还能正常运行？这些都需要验证。不过今天暂时先这样了。
